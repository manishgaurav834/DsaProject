    ...
    void groupFreightCars() {
        ...
        for(auto &it : vertices){
            ...
            for(auto car : it -> freight_cars){
                 if(car -> can_move() == true && car -> reached_city == false){
            ...
            for(auto k : unmoved_cars){
                //cout << "after3" << endl;
                if(k -> can_move() == true && k -> reached_city == false){
                 //cout << "after33" << endl;
...
public:
   ...
   std::map<std::string, std::string> get_state() {
    ...
    std::vector<std::string> get_stranded_parcels() {
       ...
       // cout << "SIZE OF THE PARCEL : " << this -> parcels.size() << endl;
        vector<string> stranded_parcels;
        for(auto it : this -> all_parcels){
           if(it -> delivered != true){
            stranded_parcels.push_back(it -> parcel_id);
           }
        }
...
//         cout << it << endl;
//     }
//     vector<string> deliver = prc.get_delivered_parcels();
//     cout << deliver.size() << endl;
